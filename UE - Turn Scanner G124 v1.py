...
def get_MDS_submission_data(): # scan the PDF for first text occurrence of Turn Deadline and extract dataframe 
    ...
    search_string = 'This weeks League selection' # set desired searchable text 
    ...
    turn_data = tabula.read_pdf(file, area=[0.9*72, 6.6*72, 1.2*72, 7.8*72], pages=page[0]) # extract data from the table
    
    # set turn deadline
    duedate = " ".join(turn_data[-1])
    #print(duedate)

    team_data = tabula.read_pdf(file, area=[0*72, 0.2*72, 1.2*72, 2.6*72], pages=page[0]) # extract data from the table
    team_id = " ".join(team_data[0])
    
    turn_data = tabula.read_pdf(file, area=[0.58*72, 3*72, 1.05*72, 5.46*72], pages=page[0])[0] # extract data from the table
...
# Add ver players to new dataframe
...
att_w_att_SA = y_mid[y_mid.SA.str.contains("Att")]
